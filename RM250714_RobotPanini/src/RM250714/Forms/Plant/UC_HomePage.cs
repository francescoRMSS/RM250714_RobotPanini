...
namespace RM.src.RM250714
{
    ...
    public partial class UC_HomePage : UserControl
    {
        ...
        {
            if (RobotManager.RobotGeneralError)
                RobotManager.RobotGeneralError = false;
            else
                RobotManager.RobotGeneralError = true;
        }

        private void button16_Click(object sender, EventArgs e)
        {
            if (RobotManager.RobotError)
                RobotManager.RobotError = false;
            else
                RobotManager.RobotError = true;
        }

        private void button17_Click(object sender, EventArgs e)
        {
            if (RobotManager.RobotKinError)
                RobotManager.RobotKinError = false;
            else
                RobotManager.RobotKinError = true;
        }

        private void button18_Click(object sender, EventArgs e)
        {
            if (RobotManager.RobotMovementError)
                RobotManager.RobotMovementError = false;
            else
                RobotManager.RobotMovementError = true;
        }

        private void button19_Click(object sender, EventArgs e)
        {
            if (RobotManager.RobotPropertiesError)
                RobotManager.RobotPropertiesError = false;
            else
        ...
        {
            if (RobotManager.RobotConnected)
                RobotManager.RobotConnected = false;
            else
                RobotManager.RobotConnected = true;
        }

        private void button28_Click(object sender, EventArgs e)
        {
            if (RobotManager.RunTimeError)
                RobotManager.RunTimeError = false;
            else
                RobotManager.RunTimeError = true;
        }

        private void button27_Click(object sender, EventArgs e)
        {
            if (RobotManager.DataError)
                RobotManager.DataError = false;
            else
                RobotManager.DataError = true;
        }

        private void button26_Click(object sender, EventArgs e)
        {
            if (RobotManager.HomeRoutineError)
                RobotManager.HomeRoutineError = false;
            else
                RobotManager.HomeRoutineError = true;
        }

        private void button25_Click(object sender, EventArgs e)
        {
            if (RobotManager.TrayNotPresentError)
                RobotManager.TrayNotPresentError = false;
            else
                RobotManager.TrayNotPresentError = true;
        }

        private void button24_Click(object sender, EventArgs e)
        {
            if (RobotManager.GripperNotClosedError)
                RobotManager.GripperNotClosedError = false;
            else
                RobotManager.GripperNotClosedError = true;
        }

        private void button23_Click(object sender, EventArgs e)
        {
            if (RobotManager.SlideNotOutError)
                RobotManager.SlideNotOutError = false;
            else
                RobotManager.SlideNotOutError = true;
        }

        private void button22_Click(object sender, EventArgs e)
        {
            if (RobotManager.SlideNotInError)
                RobotManager.SlideNotInError = false;
            else
                RobotManager.SlideNotInError = true;
        }
    } 
}
