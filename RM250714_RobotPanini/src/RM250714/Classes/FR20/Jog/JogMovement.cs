...
namespace RM.src.RM250714.Classes.FR20.Jog
{
    /// <summary>
    ...
    public static class JogMovement
    {
        ...
        /// Nome del task
        /// </summary>
        private static readonly string CheckJogRequestTaskName = nameof(CheckJogRequest);
        ...
        /// </summary>
        private static readonly int jogRobotThreadRefreshPeriod = 20;

        /// <summary>
        /// A 1 quando il thread è partito
        /// </summary>
        public static bool jogRobotTaskStarted = false;

        /// <summary>
        /// A 1 quando il thread deve essere fermato
        /// </summary>

        public static bool stopJogTask = false;
        ...
        /// </summary>
        public static void StartJogRobotTask()
        {
            if (!jogRobotTaskStarted) // Se il thread non è già avviato
            {
                RobotManager.taskManager.AddAndStartTask(CheckJogRequestTaskName, CheckJogRequest, TaskType.LongRunning, false);
                jogRobotTaskStarted = true;
                stopJogTask = false;
            }
        }
        ...
        /// </summary>
        public static void StopJogRobotTask()
        {
            if (jogRobotTaskStarted) // Se il thread è attivo
            {
                //stopJogThread = true; // Imposta il flag per fermare il thread
                RobotManager.taskManager.StopTask(CheckJogRequestTaskName);
                jogRobotTaskStarted = false;
            }
        }
        private static async Task CheckJogRequest(CancellationToken token)
        {
            ...
            try
            {
                while (!stopJogTask && !token.IsCancellationRequested)
                {
                    ...
                    await Task.Delay(jogRobotThreadRefreshPeriod);
                }
            ...
            {
                log.Error($"[TASK] {CheckJogRequestTaskName}: {ex}");
                throw;
            }
            finally
            {
